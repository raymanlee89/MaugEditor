...
import { Pointer, PointerProps } from './Pointer.tsx';
import { getWheelHandlePosition, getWheelValueFromInput } from './utils.ts';
...
const Wheel = React.forwardRef<HTMLDivElement, WheelProps>((props, ref) => {
  ...
  const hsvas = colors as [HsvaColor];
  const hexs = hsvas.map((hsva) => hsvaToHex(hsva));
  const positions = hsvas.map((hsva) => getWheelHandlePosition({ width }, hsva));
  ...
  const createPointer = (pos: {x: number; y: number;}, i: number) => {
    return(
      <Pointer
        prefixCls={prefixCls}
        style={{
  ...
  return (
    <Interactive
      ...
      onDown={handleChange}
    >
      {positions.map((pos, i) => createPointer(pos, i))}
      <div
        style={{
          position: 'absolute',
          borderRadius: '50%',
      ...
      <div
        style={{
          ...
          position: 'absolute',
          inset: 0,
          opacity: typeof hsvas[0].v === 'number' ? 1 - hsvas[0].v / 100 : 0,
        }}
      />
    </Interactive>
  );
